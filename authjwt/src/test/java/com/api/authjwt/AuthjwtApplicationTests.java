...
@Autowired
    ...
    void setup() {
        userRepository.findByUsername("admin").ifPresentOrElse(
                user -> { /* Já existe */ },
                () -> {
                    com.api.authjwt.models.Usuario admin = new com.api.authjwt.models.Usuario(null, "admin", passwordEncoder.encode("123456"), "ADMIN");
        ...
        userRepository.findByUsername("user").ifPresentOrElse(
                user -> { /* Já existe */ },
                () -> {
                    com.api.authjwt.models.Usuario regularUser = new com.api.authjwt.models.Usuario(null, "user", passwordEncoder.encode("password"), "USER");
    ...
    void testLoginSuccess() throws Exception {
        mockMvc.perform(post("/auth/login")
            ...
            // Validate the token using an assertion
            String token = mockMvc.perform(post("/auth/login")
                    .param("username", "admin")
                    .param("password", "123456")
                    .contentType(MediaType.APPLICATION_FORM_URLENCODED))
                   .andReturn().getResponse().getContentAsString();
            org.junit.jupiter.api.Assertions.assertTrue(jwtService.validateToken(token));
    }
